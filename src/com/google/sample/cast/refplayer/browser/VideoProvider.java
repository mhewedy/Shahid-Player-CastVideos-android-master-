...
public class VideoProvider {
    ...
    public JSONObject parseUrl(String urlString) {
        InputStream is = null;
        try {
            ...
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                    urlConnection.getInputStream(), "utf8"), 1024);
    ...
    public static List<MediaInfo> buildMedia(String url) throws JSONException {
        ...
        JSONArray movieList = jsonObj.getJSONArray("movieList");

        if (null != movieList) {
            for (int i = 0; i < movieList.length(); i++) {
                JSONObject movie = movieList.getJSONObject(i);
                mediaList.add(buildMediaInfo(movie.getString("title"), "", "", movie.getInt("durationSeconds"), movie.getString("videoUrl"),
                        "application/x-mpegurl", movie.getString("posterUrl"), "", null));
            }
        }
        return mediaList;
    }

    public static List<MediaInfo> buildMediaOriginal(String url) throws JSONException {
